...
static int _posix_spawn(pid_t *pid,
                        ...
                        char *const envp[])
{
  NSLog(@"posix_spawn called with %s", path);
  int result = 0;
  char **newEnvp = NULL;
  ...
  @autoreleasepool {
    ...
    if (shouldInsertLib) {
      ...
      char **newArgv = ArrayOfStringsByAppendingStrings(argv, addedArgv);

      result = posix_spawn(pid, path, file_actions, attrp, newArgv, newEnvp);
    } else {
